  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-900 tracking-tight">Practice Dashboard</h2>
          <p className="text-slate-500 text-xs">Real-time operational metrics for RuffStuff Counseling PLLC.</p>
        </div>
        {!isStandalone && (
          <button 
      ...
      {/* COMPACT STATS GRID - Added min-w-0 to prevent flex/grid collapse */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 min-w-0">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Client Traffic & Billing</h3>
            <div className="flex items-center gap-2">
               <div className="flex items-center gap-1.5">
                 <div className="w-2 h-2 rounded-full bg-blue-500" />
                 <span className="text-[10px] font-bold text-slate-400">Clients</span>
               </div>
          ...
          {/* Added relative and min-h to ensure ResponsiveContainer has valid dimensions on mount */}
      ...
      <div className="bg-slate-900 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden border border-slate-800">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2">
            <h4 className="text-lg font-black italic tracking-tight">Active Strategy Focus</h4>
            <p className="text-xs text-slate-400 max-w-2xl leading-relaxed">
              Prioritize <strong>Thriveworks</strong> 7AM-12PM slots for Week 3. Marketing Sub-COO recommends BATCH content creation for Montana Telehealth awareness campaign.
            </p>
          </div>
          <button 
            onClick={() => setActiveTab('chat')}
            className="w-full md:w-auto px-8 py-3 bg-orange-500 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-orange-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20"
          >
